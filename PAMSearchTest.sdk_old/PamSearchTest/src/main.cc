...
void WriteGuideToSD(u8 guide[29], unsigned guideCount);
...
//			}
			guideCount++;
			WriteGuideToSD(guide, guideCount);
		}
	} while (pos != -1);
...
void WriteGuideToSD(u8 guide[29], unsigned guideCount)
{
	...
	fileName[6] = ((guideCount / 100) % 10) + 48;
	fileName[7] = ((guideCount / 10) % 10) + 48;
	fileName[8] = (guideCount % 10) + 48;
	fileName[9] = '.';
	fileName[10] = 't';
	fileName[11] = 'x';
	fileName[12] = 't';
	...
	if ((fr = file.fsopen(fileName, FA_WRITE | FA_CREATE_ALWAYS)) == FR_OK) {
		xil_printf("Opened %s\r\n", fileName);
		fr = file.fswrite(guide, 29, &bytesWritten);
		if (fr == FR_OK)
