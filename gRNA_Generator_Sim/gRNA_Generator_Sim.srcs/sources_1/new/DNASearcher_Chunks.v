...
module DNASearcher_Chunks #(
    parameter chunkSize = 12,
            ...
            case (state)
                ...
                START_CHECK_SEQ : begin
                        ...
                        if (matchFound == 1'b1) begin
                            if (innerPos >= 3) begin
                                chunk1 <= {seqBig[outerPos], seqBig[outerPos-1], seqBig[outerPos-2], seqBig[outerPos-3]};
                                chunk2 <= {seqSmall[innerPos], seqSmall[innerPos-1], seqSmall[innerPos-2], seqSmall[innerPos-3]};
                                state <= CHECK_SEQ;
                            end else begin
                                state <= COMPLETE;
                            end
